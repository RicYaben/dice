...
const (
	Propagate_Trigger_FullMethodName = "/proto.Propagate/Trigger"
)

// PropagateClient is the client API for Propagate service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type PropagateClient interface {
	Trigger(ctx context.Context, in *Empty, opts ...grpc.CallOption) (*Empty, error)
}
...
func (c *propagateClient) Trigger(ctx context.Context, in *Empty, opts ...grpc.CallOption) (*Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(Empty)
	err := c.cc.Invoke(ctx, Propagate_Trigger_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// PropagateServer is the server API for Propagate service.
// All implementations must embed UnimplementedPropagateServer
// for forward compatibility.
type PropagateServer interface {
	Trigger(context.Context, *Empty) (*Empty, error)
	mustEmbedUnimplementedPropagateServer()
}

// UnimplementedPropagateServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
func (UnimplementedPropagateServer) Trigger(context.Context, *Empty) (*Empty, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Trigger not implemented")
}
func (UnimplementedPropagateServer) mustEmbedUnimplementedPropagateServer() {}
func (UnimplementedPropagateServer) testEmbeddedByValue()                   {}

// UnsafePropagateServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to PropagateServer will
// result in compilation errors.
type UnsafePropagateServer interface {
	mustEmbedUnimplementedPropagateServer()
}

func RegisterPropagateServer(s grpc.ServiceRegistrar, srv PropagateServer) {
	// If the following call pancis, it indicates UnimplementedPropagateServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
...
func _Propagate_Trigger_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Empty)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PropagateServer).Trigger(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: Propagate_Trigger_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PropagateServer).Trigger(ctx, req.(*Empty))
	}
	return interceptor(ctx, in, info, handler)
}

// Propagate_ServiceDesc is the grpc.ServiceDesc for Propagate service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var Propagate_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "proto.Propagate",
	HandlerType: (*PropagateServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Trigger",
			Handler:    _Propagate_Trigger_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "proto/module.proto",
